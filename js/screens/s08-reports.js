...
function renderReports(container, options) {
  ...
  html += '<div>';
  html += '<h1 class="text-lg font-semibold text-[#0f766e]">RBM Reports & Analytics</h1>';
  html += '<p class="text-sm text-[#6b7280]">Reserve Bank of Malawi — NDMFI 017/22 Compliance Reports</p>';
  html += '</div>';
  html += '<div class="flex gap-3">';
  html += '<button id="btn-rbm-summary" class="bg-[#0f766e] text-white px-4 py-2 rounded-xl hover:bg-[#134e4a] font-medium text-sm">📊 Summary Dashboard</button>';
  html += '<button id="btn-rbm-all" class="bg-[#111827] text-white px-4 py-2 rounded-xl hover:bg-[#047857] font-medium text-sm">📄 Generate All Reports</button>';
  html += '</div>';
  html += '</div>';
  ...
  reports.forEach(function(r) {
    ...
    html += '<div>';
    html += '<h3 class="text-sm font-semibold text-[#0f766e]">' + r.code + ' — ' + r.name + '</h3>';
    html += '<p class="text-xs text-[#6b7280] mt-1">' + r.desc + '</p>';
    html += '<p class="text-xs text-[#9ca3af] mt-1">Frequency: ' + r.freq + '</p>';
    html += '</div>';
    html += '</div>';
    html += '<button class="generate-rbm-report bg-[#0f766e] text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-[#134e4a]" data-code="' + r.code + '">Generate</button>';
    html += '</div>';
    html += '</div>';
  });
...
function renderPortfolioSummary(loans, products, branches, clients, vouchers, savingsAccounts) {
  ...
  html += '<h4 class="text-sm font-semibold text-[#0f766e] mb-3">PAR Analysis</h4>';
  html += '<div class="grid grid-cols-2 md:grid-cols-4 gap-3">';
  var totalPAR = par1_30_amt + par31_60_amt + par61_90_amt + par90plus_amt;
  html += parStatCard('PAR 1-30 Days', par1_30_amt, totalPortfolio, '#F59E0B', 5);
  ...
  // Branch summary
  html += '<div class="bg-white rounded-2xl border border-[#d1d5db] p-4 mb-6">';
  html += '<h4 class="text-sm font-semibold text-[#0f766e] mb-3">Branch Portfolio Summary</h4>';
  ...
  branches.forEach(function(b, idx) {
    ...
    html += '<td class="px-3 py-2 font-medium text-[#0f766e]' + (isMzuzu ? ' text-[#0f766e]' : '') + '">' + escapeHtml(b.branchName) + (isMzuzu ? ' <span class="text-xs text-[#111827] font-semibold">*Saile Twin Branch</span>' : '') + '</td>';
    ...
    html += '<td class="px-3 py-2 text-right text-[#F59E0B] font-semibold">' + share + '%</td>';
    html += '</tr>';
  });
  ...
  html += '<h4 class="text-sm font-semibold text-[#0f766e] mb-3">Loan Product Breakdown</h4>';
  ...
  products.forEach(function(p) {
    ...
    var sigStyle = p.isSignatureProduct ? 'border-2 border-[#F59E0B] bg-[#FFFBEB]' : '';
    html += '<div class="flex items-center justify-between p-2 rounded-lg ' + sigStyle + '">';
    html += '<div>';
    html += '<span class="text-sm font-medium text-[#0f766e]">' + escapeHtml(p.productCode) + ' - ' + escapeHtml(p.productName) + '</span>';
    if (p.isSignatureProduct) html += ' <span class="text-xs bg-[#F59E0B] text-[#92400E] px-1.5 py-0.5 rounded">⭐ SIGNATURE</span>';
    html += '</div>';
    html += '<div class="text-right">';
    html += '<div class="text-sm font-semibold text-[#0f766e]">' + formatCurrency(prodAmt) + '</div>';
...
function renderSummaryReport(loans, products, branches, clients, vouchers, savingsAccounts) {
  ...
  html += '<div class="w-10 h-10 bg-[#0f766e] rounded-lg flex items-center justify-center text-white font-bold text-lg">S</div>';
  html += '<div><h2 class="text-lg font-bold text-[#0f766e]">Saile Financial Services</h2><p class="text-xs text-[#6b7280]">NDMFI 017/22 — Portfolio Summary Report</p></div>';
  ...
  html += '<h3 class="text-sm font-semibold text-[#0f766e] mt-6 mb-3">Branch Portfolio Breakdown</h3>';
  ...
  branches.forEach(function(b) {
    ...
    html += '<tr><td class="px-3 py-1 border-b text-[#0f766e]">' + escapeHtml(b.branchName) + '</td><td class="px-3 py-1 border-b text-right font-semibold">' + formatCurrency(bPort) + '</td><td class="px-3 py-1 border-b text-right">' + bClients + '</td><td class="px-3 py-1 border-b text-right">' + bActiveLoans.length + '</td><td class="px-3 py-1 border-b text-right text-[#F59E0B] font-semibold">' + share + '%</td></tr>';
  });
  var totalShare = totalPortfolio > 0 ? (totals.portfolio / totalPortfolio * 100).toFixed(1) : 0;
  html += '<tr class="font-bold"><td class="px-3 py-2 border-t border-[#0f766e] text-[#0f766e]">TOTAL</td><td class="px-3 py-2 border-t border-[#0f766e] text-right">' + formatCurrency(totals.portfolio) + '</td><td class="px-3 py-2 border-t border-[#0f766e] text-right">' + totals.clients + '</td><td class="px-3 py-2 border-t border-[#0f766e] text-right">' + totals.loans + '</td><td class="px-3 py-2 border-t border-[#0f766e] text-right text-[#0f766e]">' + totalShare + '%</td></tr>';
...
function generateRBMSpecificReport(code, loans, products, branches, clients, vouchers, savingsAccounts) {
  ...
  switch(code) {
    ...
    case 'RBM-004':
      ...
      content += statCard('Total Savings Deposits', formatCurrency(totalSavings), savingsAccounts.length + ' accounts', '#0f766e');
      content += statCard('Voluntary Accounts', String(volCount), 'Flexible deposits', '#F59E0B');
      content += '</div>';
    ...
    case 'RBM-006':
      ...
      content = '<div class="space-y-4">';
      content += '<div class="grid grid-cols-2 gap-4">';
      content += statCard('Cash Position', formatCurrency(cashBalance), 'Liquid assets', '#0f766e');
      content += statCard('Current Liabilities', formatCurrency(totalCurrentLiability), 'Monthly maturities', '#111827');
      content += '</div>';
      content += '<div class="bg-[#F9FAFB] rounded-xl p-4 text-center">';
      content += '<div class="text-sm text-[#6b7280] mb-1">Liquidity Ratio</div>';
      content += '<div class="text-4xl font-bold text-[#0f766e]">' + liquidityRatio + '%</div>';
