...
function renderUsers(container, options) {
  ...
  function render() {
    ...
    html += '<div>';
    html += '<h1 class="text-lg font-semibold text-[#1E3A8A]">Saile User Management</h1>';
    html += '<p class="text-sm text-[#6B7280]">' + users.length + ' registered users</p>';
    html += '</div>';
    if (!readOnly) {
      html += '<button id="btn-add-user" class="bg-[#1F2937] text-white px-6 py-2.5 rounded-xl hover:bg-[#1F2937] font-medium text-sm">+ New User</button>';
    }
    html += '</div>';
    ...
    if (users.length === 0) {
      ...
      for (var i = 0; i < users.length; i++) {
        var u = users[i];
        var statusClass = u.status === 'Active' ? 'bg-[#f0fdf4] text-[#1E3A8A]' : 'bg-[#FEF2F2] text-[#991B1B]';
        var roleLabel = u.role.replace('_', ' ');

        html += '<tr class="border-b border-[#d1d5db] hover:bg-[#f4f4f5]">';
        html += '<td class="px-4 py-3 font-medium text-[#1E3A8A]">' + escapeHtml(u.fullName) + '</td>';
        ...
        if (!readOnly) {
          if (u.status === 'Active') {
            ...
            html += '<button class="text-[#1E3A8A]merald-600 text-xs font-medium hover:underline btn-activate-user" data-id="' + u.id + '">Activate</button>';
          }
...
function renderUserForm(container, options) {
  ...
  html += '<button id="btn-back-users" class="bg-white border border-[#d1d5db] text-[#1E3A8A] px-4 py-2 rounded-xl hover:bg-[#f4f4f5] text-sm">&larr; Back</button>';
  html += '<h1 class="text-lg font-semibold text-[#1E3A8A]">Create New User</h1>';
  html += '</div>';
  ...
  html += '<input type="text" name="fullName" class="w-full px-3 py-2.5 border border-[#d1d5db] rounded-xl focus:ring-2 focus:ring-[#1E3A8A] focus:border-[#1E3A8A] text-sm" required></div>';
  html += '<div><label class="block text-sm font-medium text-[#1E3A8A] mb-1.5">Email *</label>';
  html += '<input type="email" name="email" class="w-full px-3 py-2.5 border border-[#d1d5db] rounded-xl focus:ring-2 focus:ring-[#1E3A8A] focus:border-[#1E3A8A] text-sm" required></div>';
  html += '</div>';

  html += '<div class="grid grid-cols-1 md:grid-cols-2 gap-4">';
  html += '<div><label class="block text-sm font-medium text-[#1E3A8A] mb-1.5">Password *</label>';
  html += '<input type="password" name="password" class="w-full px-3 py-2.5 border border-[#d1d5db] rounded-xl focus:ring-2 focus:ring-[#1E3A8A] focus:border-[#1E3A8A] text-sm" required></div>';
  html += '<div><label class="block text-sm font-medium text-[#1E3A8A] mb-1.5">Role *</label>';
  ...
  html += '<div><label class="block text-sm font-medium text-[#1E3A8A] mb-1.5">PIN (for field officers, 4 digits)</label>';
  html += '<input type="text" name="pin" maxlength="4" pattern="[0-9]{4}" class="w-full px-3 py-2.5 border border-[#d1d5db] rounded-xl focus:ring-2 focus:ring-[#1E3A8A] focus:border-[#1E3A8A] text-sm" placeholder="Optional"></div>';

  html += '<div class="flex gap-3">';
  html += '<button type="submit" class="bg-[#1F2937] text-white px-6 py-2.5 rounded-xl hover:bg-[#1F2937] font-medium text-sm">Create User</button>';
  html += '<button type="button" id="btn-cancel-user" class="bg-white border border-[#d1d5db] text-[#1E3A8A] px-6 py-2.5 rounded-xl hover:bg-[#f4f4f5] font-medium text-sm">Cancel</button>';
  html += '</div></form></div>';
