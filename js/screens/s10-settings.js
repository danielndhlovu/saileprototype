...
function renderSettings(container, options) {
  ...
  function render() {
    ...
    html += '<div>';
    html += '<h1 class="text-lg font-semibold text-[#0f766e]">System Configuration</h1>';
    ...
    for (var t = 0; t < tabs.length; t++) {
      var active = viewMode === tabs[t][0] ? 'bg-[#0f766e] text-white' : 'bg-white text-[#0f766e] border border-[#d1d5db] hover:bg-[#f4f4f5]';
  ...
  function renderGeneralSettings(readOnly) {
    ...
    html += '<h3 class="text-sm font-semibold text-[#0f766e]">Institution Information</h3>';

    html += '<div class="grid grid-cols-1 md:grid-cols-2 gap-4">';
    html += '<div><label class="block text-sm font-medium text-[#0f766e] mb-1.5">Institution Name</label>';
    html += '<input type="text" id="setting-institution" class="w-full px-3 py-2.5 border border-[#d1d5db] rounded-xl text-sm focus:ring-2 focus:ring-[#0f766e] focus:border-[#0f766e]" value="' + escapeHtml(settings.institutionName || '') + '"' + (readOnly ? ' disabled' : '') + '></div>';

    html += '<div><label class="block text-sm font-medium text-[#0f766e] mb-1.5">RBM License Number</label>';
    html += '<input type="text" id="setting-license" class="w-full px-3 py-2.5 border border-[#d1d5db] rounded-xl text-sm focus:ring-2 focus:ring-[#0f766e] focus:border-[#0f766e]" value="' + escapeHtml(settings.institutionLicense || '') + '"' + (readOnly ? ' disabled' : '') + '></div>';

    html += '<div><label class="block text-sm font-medium text-[#0f766e] mb-1.5">Language</label>';
    html += '<select id="setting-language" class="w-full px-3 py-2.5 border border-[#d1d5db] rounded-xl text-sm focus:ring-2 focus:ring-[#0f766e] focus:border-[#0f766e]"' + (readOnly ? ' disabled' : '') + '>';
    ...
    html += '<div><label class="block text-sm font-medium text-[#0f766e] mb-1.5">Currency</label>';
    html += '<input type="text" id="setting-currency" class="w-full px-3 py-2.5 border border-[#d1d5db] rounded-xl text-sm focus:ring-2 focus:ring-[#0f766e] focus:border-[#0f766e]" value="' + escapeHtml(settings.currency) + '"' + (readOnly ? ' disabled' : '') + '></div>';

    html += '<div><label class="block text-sm font-medium text-[#0f766e] mb-1.5">Session Timeout (minutes)</label>';
    html += '<input type="number" id="setting-timeout" class="w-full px-3 py-2.5 border border-[#d1d5db] rounded-xl text-sm focus:ring-2 focus:ring-[#0f766e] focus:border-[#0f766e]" value="' + settings.sessionTimeoutMinutes + '"' + (readOnly ? ' disabled' : '') + '></div>';

    html += '<div><label class="flex items-center gap-2">';
    html += '<input type="checkbox" id="setting-rbm" class="w-4 h-4 rounded border-[#d1d5db] text-[#0f766e] focus:ring-[#0f766e]"' + (settings.rbmReportingEnabled !== false ? ' checked' : '') + (readOnly ? ' disabled' : '') + '>';
    html += '<span class="text-sm text-[#0f766e]">Enable RBM Reporting Module</span></label></div>';

    html += '<div><label class="flex items-center gap-2">';
    html += '<input type="checkbox" id="setting-sms" class="w-4 h-4 rounded border-[#d1d5db] text-[#0f766e] focus:ring-[#0f766e]"' + (settings.smsGatewayEnabled ? ' checked' : '') + (readOnly ? ' disabled' : '') + '>';
    html += '<span class="text-sm text-[#0f766e]">Enable SMS Gateway (Phase 3)</span></label></div>';
    html += '</div>';

    // Role Switcher
    html += '<div class="pt-4 border-t border-[#d1d5db]">';
    html += '<h4 class="text-sm font-medium text-[#0f766e] mb-3">Demo Role Switcher</h4>';
    html += '<div class="flex flex-wrap gap-2">';
    var roles = [
    ...
    for (var r = 0; r < roles.length; r++) {
      ...
      var btnClass = isActive ? 'bg-[#111827] text-white' : 'bg-white border border-[#d1d5db] text-[#0f766e] hover:bg-[#f4f4f5]';
    ...
    if (!readOnly) {
      html += '<button id="btn-save-settings" class="bg-[#111827] text-white px-6 py-2.5 rounded-xl hover:bg-[#047857] font-medium text-sm mt-4">Save Settings</button>';
    }
    html += '</div>';
    return html;
  }
  ...
  function renderProductSettings(readOnly) {
    ...
    if (!readOnly) {
      html += '<div class="flex justify-end"><button id="btn-add-product" class="bg-[#111827] text-white px-4 py-2 rounded-xl hover:bg-[#047857] font-medium text-sm">+ New Product</button></div>';
    }
    ...
    for (var i = 0; i < products.length; i++) {
      ...
      html += '<h4 class="text-sm font-semibold text-[#0f766e]">' + escapeHtml(p.productCode) + ' — ' + escapeHtml(p.productName) + '</h4>';
  ...
  function renderBranchSettings(readOnly) {
    ...
    if (!readOnly) {
      html += '<div class="flex justify-end"><button id="btn-add-branch" class="bg-[#111827] text-white px-4 py-2 rounded-xl hover:bg-[#047857] font-medium text-sm">+ New Branch</button></div>';
    }
    ...
    for (var i = 0; i < branches.length; i++) {
      ...
      html += '<div class="bg-white rounded-2xl border border-[#d1d5db] p-4 ' + (isMzuzu ? 'border-[#0f766e] border-2' : '') + '">';
      html += '<div class="flex items-start justify-between">';
      html += '<div>';
      html += '<h4 class="text-sm font-semibold text-[#0f766e]">' + escapeHtml(b.branchName) + '</h4>';
      ...
      if (b.managerName) {
        ...
        html += '<div class="font-medium text-[#0f766e]">' + escapeHtml(b.managerName) + '</div>';
  ...
  function renderPenaltySettings(readOnly) {
    ...
    html += '<h3 class="text-sm font-semibold text-[#0f766e] mb-4">Penalty Rules (per Product)</h3>';
    ...
    products.forEach(function(p) {
      ...
      html += '<td class="px-3 py-2 text-right text-[#111827] font-semibold">' + formatPercentage(p.latePenaltyRate) + '/day</td>';
...
function renderMigration(container, options) {
  ...
  html += '<div><h2 class="text-lg font-semibold text-[#0f766e]">Data Migration Module</h2>';
  ...
  if (!readOnly) {
    html += '<button id="btn-run-migration" class="bg-[#0f766e] text-white px-6 py-2.5 rounded-xl hover:bg-[#134e4a] font-medium text-sm">Run Migration Demo</button>';
  }
  html += '</div>';

  // Migration status
  html += '<div id="migration-status" class="hidden space-y-3">';
  html += '<h4 class="text-sm font-semibold text-[#0f766e]">Migration Progress</h4>';
  ...
  html += '<div class="w-full bg-gray-200 rounded-full h-2"><div id="migration-bar" class="bg-[#0f766e] h-2 rounded-full transition-all duration-500" style="width:0%"></div></div>';
...
function migrationStepCard(step, title, desc, icon) {
  return '<div class="bg-[#F5F6FA] rounded-xl p-4 text-center">' +
    ...
    '<div class="text-sm font-semibold text-[#0f766e] mb-1">Step ' + step + ': ' + title + '</div>' +
