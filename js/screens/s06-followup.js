...
function renderFollowup(container, options) {
  ...
  function render() {
    ...
    html += '<div>';
    html += '<h1 class="text-lg font-semibold text-[#1E3A8A]">Saile Follow-up & Tracking</h1>';
    html += '<p class="text-sm text-[#6B7280]">' + followups.length + ' overdue accounts</p>';
    html += '</div>';
    if (!readOnly) {
      html += '<button id="btn-new-followup" class="bg-[#1F2937] text-white px-6 py-2.5 rounded-xl hover:bg-[#152C5B] font-medium text-sm">+ New Follow-up</button>';
    }
    html += '</div>';

    // View toggle
    html += '<div class="flex gap-2">';
    html += '<button class="followup-view-tab px-4 py-2 rounded-xl text-sm font-medium ' + (viewMode === 'list' ? 'bg-[#1F2937] text-white' : 'bg-white text-[#1E3A8A] border border-[#d1d5db]') + '" data-view="list">Overdue List</button>';
    html += '<button class="followup-view-tab px-4 py-2 rounded-xl text-sm font-medium ' + (viewMode === 'history' ? 'bg-[#1F2937] text-white' : 'bg-white text-[#1E3A8A] border border-[#d1d5db]') + '" data-view="history">History</button>';
    html += '</div>';
    ...
    if (viewMode === 'list') {
      ...
      for (var i = 0; i < sorted.length; i++) {
        var f = sorted[i];
        var dpdClass = f.daysPastDue > 90 ? 'bg-[#FEF2F2] text-[#991B1B]' :
                       f.daysPastDue > 30 ? 'bg-amber-50 text-amber-700' : 'bg-sky-50 text-sky-700';
        var outcomeClass = f.contactOutcome === 'promised_to_pay' ? 'text-[#1E3A8A]merald-600' :
                           f.contactOutcome === 'client_relocated' ? 'text-[#DC2626]' : 'text-amber-600';

        html += '<div class="bg-white rounded-2xl border border-[#d1d5db] p-4">';
        html += '<div class="flex items-start justify-between">';
        html += '<div class="flex-1">';
        html += '<h4 class="text-sm font-medium text-[#1E3A8A]">' + escapeHtml(f.clientName) + '</h4>';
        ...
        html += '<button class="flex items-center gap-1 px-3 py-1.5 bg-[#1E3A8A]merald-50 text-[#1E3A8A]merald-700 rounded-lg text-xs font-medium hover:bg-[#1E3A8A]merald-100 btn-call-client" data-phone="' + escapeHtml(f.clientName) + '">📞 Call</button>';
        ...
        if (!readOnly) {
          html += '<button class="flex items-center gap-1 px-3 py-1.5 bg-[#f4f4f5] text-[#1E3A8A] rounded-lg text-xs font-medium hover:bg-gray-200 btn-update-followup" data-id="' + f.id + '">✏️ Update</button>';
        }
        html += '</div></div>';
      }
      html += '</div>';
      ...
      for (var h = 0; h < sorted.length; h++) {
        ...
        html += '<td class="px-4 py-3 font-medium text-[#1E3A8A]">' + escapeHtml(fh.clientName) + '</td>';
...
function renderFollowupForm(container, followupId, options) {
  ...
  html += '<button id="btn-back-followup" class="bg-white border border-[#d1d5db] text-[#1E3A8A] px-4 py-2 rounded-xl hover:bg-[#f4f4f5] text-sm">&larr; Back</button>';
  html += '<h1 class="text-lg font-semibold text-[#1E3A8A]">' + (existing ? 'Update Follow-up' : 'New Follow-up') + '</h1>';
  html += '</div>';
  ...
  html += '<div><label class="block text-sm font-medium text-[#1E3A8A] mb-1.5">Days Past Due</label>';
  html += '<input type="number" name="daysPastDue" class="w-full px-3 py-2.5 border border-[#d1d5db] rounded-xl focus:ring-2 focus:ring-[#1E3A8A] focus:border-[#1E3A8A] text-sm" value="' + (existing ? existing.daysPastDue : '') + '" required></div>';
  html += '<div><label class="block text-sm font-medium text-[#1E3A8A] mb-1.5">Scheduled Date</label>';
  html += '<input type="date" name="scheduledDate" class="w-full px-3 py-2.5 border border-[#d1d5db] rounded-xl focus:ring-2 focus:ring-[#1E3A8A] focus:border-[#1E3A8A] text-sm" value="' + (existing ? existing.scheduledDate : todayISO()) + '"></div>';
  html += '</div>';

  html += '<div class="grid grid-cols-1 md:grid-cols-2 gap-4">';
  html += '<div><label class="block text-sm font-medium text-[#1E3A8A] mb-1.5">Action</label>';
  ...
  html += '</select></div>';
  html += '<div><label class="block text-sm font-medium text-[#1E3A8A] mb-1.5">Contact Outcome</label>';
  ...
  html += '<textarea name="notes" rows="3" class="w-full px-3 py-2.5 border border-[#d1d5db] rounded-xl focus:ring-2 focus:ring-[#1E3A8A] focus:border-[#1E3A8A] text-sm">' + escapeHtml(existing ? existing.notes || '' : '') + '</textarea></div>';

  html += '<div class="flex gap-3">';
  html += '<button type="submit" class="bg-[#1F2937] text-white px-6 py-2.5 rounded-xl hover:bg-[#152C5B] font-medium text-sm">' + (existing ? 'Update' : 'Create Follow-up') + '</button>';
  html += '<button type="button" id="btn-cancel-followup" class="bg-white border border-[#d1d5db] text-[#1E3A8A] px-6 py-2.5 rounded-xl hover:bg-[#f4f4f5] font-medium text-sm">Cancel</button>';
  html += '</div></form></div>';
